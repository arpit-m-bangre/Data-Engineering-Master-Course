...
-- Insert values for better result and understanding

-- gpt inserted response 
INSERT INTO student_dumy2
    (id, name, sub, marks, age)
VALUES
    -- ========================================================
    -- MATHS
    -- ========================================================
    ...
    -- ========================================================
    -- RECORDS TO TEST WHERE FILTER (marks <= 35)
    -- ========================================================
    ...
    -- ========================================================
    -- NAMES WITHOUT 'R' TO TEST LIKE FILTER
    -- ========================================================
    (46, 'aman',        'Maths',    40, 22),
...
-- STEP 1: First decide which table contains the data.
FROM student_dumy2

-- STEP 2: Now filter individual rows.
-- Marks > 35 is a row-level condition.
-- Name contains 'R' is also a row-level condition.
-- Therefore, both go into WHERE.
WHERE marks > 35
  AND name LIKE '%r%'

-- STEP 3: The question says "subject-wise".
-- Subject-wise means we need one group for each subject.
GROUP BY sub

-- STEP 4: Now decide what we want to display.
-- We need: 1. Subject, 2. Average marks, 3. Student count
SELECT
    sub,
    AVG(marks) AS avg_marks,
    COUNT(id)  AS student_count

-- STEP 5: Finally, filter the groups.
-- AVG(marks) and COUNT(id) are aggregate results,
...
---------------------------------------------------------------
WHERE:
    Can filter using a column even if that column is
    NOT displayed in SELECT.
...
GROUP BY:
    ...
    Non-aggregate columns selected with GROUP BY
    generally need to be included in GROUP BY.

HAVING:
    Filters groups / aggregate results.
    It can use aggregate functions such as:
    SUM(), AVG(), COUNT(), MIN(), MAX()
    The column/expression used in HAVING does not
    necessarily have to be displayed in SELECT.

ORDER BY:
    Sorts the final result.
    A column used for sorting does not necessarily
    have to be displayed in SELECT in normal queries.
...
/*
ANSWER:
No additional clause is required here.

Reason:
1. WHERE    -> Not required because we are not filtering any rows.
2. GROUP BY -> Not required because we are not creating groups.
3. HAVING   -> Not required because there is no GROUP BY and we are not filtering any aggregate result.
4. ORDER BY -> Not required because we are not sorting the result.

MAX(salary) is an aggregate function and returns one output value for the entire table.

So this query only needs:
SELECT
FROM
*/
...
/* ============================================================
16. TASK BY SUMEDHA MA'AM - SCENARIO-BASED QUESTIONS
   ============================================================ */

/*
NOTE: Do it in Notepad or Sticky Note.
*/
