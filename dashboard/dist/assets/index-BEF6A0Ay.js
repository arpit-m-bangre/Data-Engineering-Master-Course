(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),t.credentials=e.crossOrigin===`use-credentials`?`include`:e.crossOrigin===`anonymous`?`omit`:`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var e=`====================================================================
      ...
      [STATUS: PIPPO DE COURSE FOCUS | SUNDAY RECHARGE & SYNC 🔥]
====================================================================

[!] MISSION OBJECTIVE: Sunday System Sync, Rest & Schedule Lock for ETL Module 03 (Launch Mon 07 Sep)!

[🎯 CURRENT TARGET]: SQL Full Revision + 70-Q YouTube Series Mastery & Zero Backlog (Deadline: 13-Sep-2026 10:30 AM)

TOTAL STUDY TIME TODAY: 0.25 Hours (0.25h System Sync | Sunday No Live Class)

--------------------------------------------------------------------
                    PIPPO DE COURSE STUDY SPRINTS
--------------------------------------------------------------------
[-] 09:00 AM - 10:30 AM : Sprint 1 — ETL Foundations & 5-Pillar Architecture (Deferred to Mon 07 Sep) 📦
    -> Deferred to Monday 07 Sep morning sprint.
    -> File: 03_ETL_DATA_PIPELINES/01_CLASS_AND_REVISION/2026-09-05_ETL_FOUNDATIONS.md.

[-] 10:30 AM - 11:00 AM : Morning Rest & Recharge ☕🌿
    -> Screen-off, hydration, and light stretch.

[-] 11:00 AM - 12:45 PM : Sprint 2 — Medallion Lakehouse, 4 Class Tasks & 14 Practice Drills (Deferred to Mon 07 Sep) 📦
    -> Deferred to Monday 07 Sep morning sprint.
    -> File: 03_ETL_DATA_PIPELINES/02_TASKS_AND_DRILLS/2026-09-05_TASKS_AND_DRILLS.md.
...
--------------------------------------------------------------------
[-] 09:00 PM - 10:15 PM : Sunday Break (No Live Class Scheduled) 🎓🌿
    -> Sunday faculty holiday — No live lecture scheduled.

[x] 10:15 PM - 10:30 PM : Sprint 3 — System Sync & GitHub Push (Streak Day 33 🔥) 🚀
...
====================================================================
*Rules: Pure DE Course Focus | Zero SQL Backlog | ETL Locks Mon 07 Sep*
*Daily Quote: "Consistency is keeping the streak alive every single day."*
====================================================================
`,t=`https://raw.githubusercontent.com/arpit-m-bangre/Data-Engineering-Master-Course/main/TODAYS_TASKS.txt`;function n(e){return!e||typeof e!=`string`?``:e.replace(/\r\n/g,`
`).replace(/\r/g,`
...
`),n=[],i=null;for(let e of t){let t=e.trim();if(!t)continue;let a=t.match(/^DAILY MISSION:\s*(.+)$/i);if(a){i&&=(n.push(i),null),n.push({type:`mission_header`,text:a[1].trim()});continue}let o=t.match(/^\[(?:🎯\s*)?(?:CURRENT\s+)?TARGET\]:\s*(.+)$/i)||t.match(/^CURRENT TARGET:\s*(.+)$/i);if(o){i&&=(n.push(i),null),n.push({type:`target`,text:o[1].trim()});continue}let s=t.match(/^\[!\]\s*(.+)$/);if(s){i&&=(n.push(i),null),n.push({type:`notice`,title:s[1].trim()});continue}if(/^[=\-*]{4,}$/.test(t)){i&&=(n.push(i),null);continue}let c=t.match(/^TOTAL (?:STUDY|ENGINEERING)[^:]*:\s*(.+)$/i)||t.match(/^TOTAL [^:]*TIME[^:]*:\s*(.+)$/i);if(c){i&&=(n.push(i),null),n.push({type:`stats`,text:c[1].trim()});continue}if(t.startsWith(`*Note:`)||t.startsWith(`*`)&&!t.startsWith(`**`)&&!t.includes(`[`)){i&&=(n.push(i),null),n.push({type:`note`,title:t.replace(/^\*+|\*+$/g,``).trim()});continue}let l=t.match(/^\[([xX\s\-]?)\]\s*([\d: APM–\-]+(?:AM|PM))\s*:\s*(.+)$/);if(l){i&&n.push(i);let e=l[1].trim().toLowerCase(),t=l[2].trim(),a=l[3].trim(),o=r(a);i=e===`x`?{type:`task`,status:`done`,time:t,title:a,details:[]}:e===`-`||e===`–`?o?{type:`break`,status:`break`,time:t,title:a,details:[]}:{type:`task`,status:`deferred`,time:t,title:a,details:[]}:{type:`task`,status:`pending`,time:t,title:a,details:[]};continue}if(t.startsWith(`->`)&&i){i.details.push(t.slice(2).trim());continue}if(/^\[(?:MUST-WIN|SHOULD-WIN|BONUS)[^\]]*\]/i.test(t)){i&&=(n.push(i),null),n.push({type:`mustwin`,text:t});continue}if(/^[A-Z0-9\s&—\-_:()]+$/.test(t)&&t.length>3&&!t.startsWith(`DAILY MISSION`)&&!t.startsWith(`[STATUS`)){i&&=(n.push(i),null),n.push({type:`section`,title:t});continue}}return i&&n.push(i),n}function a(e,t){let n=[];try{n=i(e)}catch(e){console.error(`[renderTasks] parseTasks failed:`,e),t.innerHTML=`<p class="empty-state">Error parsing tasks. Check TODAYS_TASKS.txt format.</p>`;return}if(n.length===0){t.innerHTML=`<p class="empty-state">No tasks found for today.</p>`;return}t.innerHTML=``;let r=n.find(e=>e.type===`mission_header`),a=document.getElementById(`date-display`);a&&r&&(a.textContent=`🗓️ ${r.text}`);let s=n.filter(e=>e.type===`task`);n.filter(e=>e.type===`break`),n.filter(e=>e.type===`notice`);function c(){let e=s.filter(e=>e.status===`done`),t=s.filter(e=>e.status===`pending`),n=s.filter(e=>e.status===`deferred`),r=e.length+t.length,i=r>0?Math.round(e.length/r*100):100;return{doneCount:e.length,pendingCount:t.length,deferredCount:n.length,activeTotal:r,pct:i}}let l=c(),u=n.find(e=>e.type===`target`),d=u?u.text.trim():``;if(u&&!/^(?:none|no\s+active|idle|n\/a)$/i.test(d)&&d.length>0){let e=document.createElement(`div`);e.className=`target-banner-card active`,e.innerHTML=`
    ...
    `}function m(){f&&p(f,c())}n.forEach(e=>{if(e.type===`target`||e.type===`mustwin`||e.type===`mission_header`)return;if(![`section`,`notice`,`stats`,`note`,`break`,`task`].includes(e.type)){console.warn(`[renderTasks] Unknown item type skipped:`,e.type,e);return}let n=document.createElement(`div`);if(e.type===`section`)n.className=`section-header-card`,n.innerHTML=`
