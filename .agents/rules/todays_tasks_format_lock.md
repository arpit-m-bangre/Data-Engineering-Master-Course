...
# Constitutional Law #13 Implementation — Parser Contract & Vercel Sync
...
TOTAL STUDY TIME TODAY: 9.0 Hours (Core SQL: 1.75h | Recall: 1.25h | Project: 1.75h | Drills: 1.25h | Authority: 1.0h | English: 0.50h | Live Class: 1.25h | Night Sync: 0.25h)
...
--------------------------------------------------------------------
[ ] 10:15 PM - 10:30 PM : Sprint N — Creative Recharge & 210-Day IG/Shorts Publishing (0.0 Study Hr Impact) 📱
    -> Publish today's scheduled 15-min technical asset from EXTRA_MAGICAL_FILES/IG_CONTENT_PLAN/
    -> Zero perfectionist loop: 1 Reel / Carousel / Infographic / Quiz Story published to IG, Shorts & LinkedIn
    -> [DoD]: Asset published & saved to profile highlight | 0.0 study hr impact (creative fun)

[ ] 10:30 PM - 10:45 PM : Final System Sync, GitHub Push & Vercel Trigger (Streak Day N 🔥) 🚀
...
## SECTION 2: PARSER CONTRACT — VERCEL DASHBOARD COMPLIANCE
...
| Marker | Condition | Parsed As | UI Card Style |
| :---: | :--- | :--- | :--- |
| `[x]` | Any text | `done` | ✅ Green completed card |
| `[ ]` | Any text | `pending` | ⬜ White active sprint card |
| `[-]` | Contains break keyword | `break` | ☕ Dimmed recharge card |
| `[-]` | No break keyword | `deferred` | ⏳ Yellow deferred card |
...
### 2.4 Sub-Bullet Detection Regex
```javascript
/^\s{4}->\s(.+)$/
```
*(Must be indented with exactly 4 spaces followed by `-> `)*.

---

## SECTION 3: SYNTAX RULES (NON-NEGOTIABLE)

1. **Card Header Format:** `[STATUS] HH:MM AM - HH:MM PM : Sprint Title`
2. **Sub-Bullets:** Exactly 4 spaces + `-> ` (`    -> Description`)
3. **Breaks:** Use `[-]` with break keyword in title.
4. **Deferred:** Use `[-]` without break keyword.
5. **Done:** Use `[x]` (lowercase/uppercase).
6. **Pending:** Use `[ ]` (single space inside brackets).
7. **Dividers:** Outer box `====================================================================` (68 chars); section dividers `--------------------------------------------------------------------`.

---

## SECTION 4: VERCEL DEPLOY PIPELINE

After updating `TODAYS_TASKS.txt`, run in `DASHBOARD/`:
```powershell
node copy_tasks.cjs
npm run build
```
Then from root (`d:\DE COURSE\`):
```powershell
$env:GCM_INTERACTIVE="never"
git add .
git commit -m "sync(dashboard): daily mission sync"
...
```text
❌ - [ ] 10:00 AM - 11:00 AM : Task (bare bullet before bracket)
❌ [ ]10:00 AM - 11:00 AM : Task (no space after bracket)
❌ [X ] 10:00 AM : Task (no closing time)
❌   -> Detail (2-space indent)
❌ 	-> Detail (tab indent)
❌ -> Detail (no indent)
❌ [-] Sprint 3 (Deferred) (no time range)
```
